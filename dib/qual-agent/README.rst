...
.. note::
   ...
   This element only *bakes* the binary, its CA trust anchor, its config, the
   systemd unit, and (once folded in) the ``qual_hold`` clean-step gate. See
   ``docs/qual-agent-implementation-plan.md`` for the full build-out plan, the
   agent⇄ramdisk contract, and the sandbox test loop.
